...
def transform_inputs(tweets, tf_vectorizer, idf_vector, pos_vectorizer):
    ...
    pos_array = pos_vectorizer.fit_transform(pd.Series(pos_tags)).toarray()
...
def get_tweets_predictions(tweets, perform_prints=True):
    ...
    model = joblib.load('../src/model_3.pkl')

    print("Loading other information...")
    tf_vectorizer = jb.load('final_tfidf.pkl')
    idf_vector = jb.load('final_idf.pkl')
    pos_vectorizer = jb.load('../src/pos_vect.pkl')
    # Load ngram dict
    # Load pos dictionary
...
if __name__ == '__main__':
    ...
    df = pd.read_csv('trump_tweets_u.csv')
